...
requireText(client, '请选择一个基金类别', 'ordinary users need a plain-language category starting point')
requireText(categoryPresets, '主动权益（大盘）', 'recommendation page must expose common category presets')
...
requireText(client, "from '@/lib/fund-category-presets'", 'recommendation page must use the shared category presets')
