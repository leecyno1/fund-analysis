...
PGDATA_DIR="${PGDATA_DIR:-/private/tmp/fund-analysis-pg}"
PGPORT="${PGPORT:-5432}"
DB_NAME="${DB_NAME:-fund_analysis}"
DB_USER="${DB_USER:-postgres}"
DATABASE_URL="${DATABASE_URL:-postgresql://postgres:fundanalysis2024@localhost:${PGPORT}/${DB_NAME}}"
...
if ! pg_isready -h localhost -p "$PGPORT" -d "$DB_NAME" >/dev/null 2>&1; then
...
PYTHON_BIN=""
for candidate in "${BACKEND_PYTHON:-}" "/opt/anaconda3/bin/python" "/usr/local/bin/python3" "/opt/homebrew/bin/python3" "$(command -v python3 || true)"; do
