...
- M1 调度通电 + 本机生产化 ✓（launchd 常驻、每日备份、每日评价快照积累）
- M2 评价数据攻坚 ✓（风格快照/评价历史/持仓覆盖真实产出）
- M3 覆盖攻坚由每日调度自动积累中
- M4 组合构建 MVP ✓（准入/权重/穿透，`/portfolio`）
- M5 基础回测 + 监控 + 交易清单 + ADR-0004 ✓（解释性回测、同类组偏离、申赎清单研究输出）
- M6 上线验收 ✓（报告 `docs/plans/2026-08-19-m6-launch-acceptance-report.md`：launchd 巡检、睡眠唤醒补跑验证、备份恢复演练 7 表一致、备份内容自检加固）

上线迭代完成，系统以本机生产形态持续运行；后续仅余长期任务（M3 覆盖率由每日调度自动积累）。
